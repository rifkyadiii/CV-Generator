...
<body class="min-h-screen flex items-center justify-center">
  ...
  <div class="bg-white p-8 rounded-lg shadow-md w-full max-w-md">
    ...
    <form method="POST" class="grid gap-4" action={{route('ApplyUpdateProjectForm' , ['id' => $project->project_id])}}>
	  @csrf
	  @method('put')
      ...
      <div>
        ...
        <input type="text" id="ToolsForProjects" name="ToolsForProjects" placeholder="Enter your full name" 
               class="form-input mt-1 w-full border-gray-300 rounded-md shadow-sm"
			   value="{{old('ToolsForProjects',$project->ToolsForProjects)}}">
      </div>
        ...
        <div>
          ...
          <textarea 
			...
			name="Projectdescription">{{old('Projectdescription',$project->Projectdescription)}}</textarea>
        </div>
